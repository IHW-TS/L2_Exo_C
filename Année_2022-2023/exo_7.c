...
void print_xpm_header(int w, int h, int c, int p) {
    printf("/* XPM */\n");
    printf("static char *gradient_xpm[] = {\n");
    printf("/* columns rows colors chars-per-pixel */\n");
    printf("\"%d %d %d %d\",\n", w, h, c, p);
}

void print_xpm_colors(pixel* gradient_colors, int len) {
    for (int i = 0; i < len; i++) {
        printf("\"%c c #%02X%02X%02X\",\n", 'A' + i, gradient_colors[i].rgb.r, gradient_colors[i].rgb.g, gradient_colors[i].rgb.b);
    }
}

void print_xpm_pixels(int w, int h) {
    for (int i = 0; i < h; i++) {
        printf("\"");
        for (int j = 0; j < w; j++) {
            printf("%c", 'A' + j);
...
int main() {
    ...
    pixel gradient_colors[SIZE];
    gradient(start, stop, gradient_colors, SIZE);

    print_xpm_header(SIZE, SIZE, SIZE, 1);
    print_xpm_colors(gradient_colors, SIZE);
    print_xpm_pixels(SIZE, SIZE);
    print_xpm_footer();
