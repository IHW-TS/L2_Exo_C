...
int main(int argc, char *argv[])
{
    ...
    if ((a = opendir(".")) == NULL)
    {
        perror("Erreur"); // affiche la dernière erreur rencontrée durant un appel système ou une fonction de bibliothèque
        exit(-1);
    }
    else
    {
...
{
    struct stat stats;
    if (stat(nom_du_fichier, &stats) == -1) // stat a deux valeurs de retour soit 0 si il a reussit l'execution, soit -1 et renvoie donc une erreur?
    {
...
void list(char *nom_du_fichier, struct stat *statinfo)
{
    mode_t st_mode = statinfo->st_mode; // st_mode => autorisations actuelles sur le fichier.
